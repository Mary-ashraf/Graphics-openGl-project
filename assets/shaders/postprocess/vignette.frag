...
void main(){
    ...
    float dist = ((tex_coord.x-0.5)*(tex_coord.x-0.5) + (tex_coord.y-0.5)*(tex_coord.y-0.5));
    vec4 frag_color_temp = texture(tex, tex_coord);
    
    frag_color = frag_color_temp/(1+4*dist);
}